...
func TestDecompressorWriterCompressReader(t *testing.T) {
	samples := getDataSamples()
	optsList := getCombinations()
	...
	for _, sample := range samples {
		for _, opts := range optsList {
			...
			repeat(REPEAT_COUNT, func(index int) {
				name := fmt.Sprintf("%s l:%d w:%d h:%d s:%d index:%d", sample.name, opts.Level(), opts.WindowBits(), opts.Header(), opts.Strategy(), index)
				t.Run(name, func(t *testing.T) {
					t.Log("Testing", name)
					t.Logf("Original data: %x", sample.decompressed)

					compressed := synchronousCompressReader(t, sample.decompressed, opts)
					t.Logf("Compressed data 1: %x", compressed)

					compressed2, decompressed := asynchronousDecompressAndCompress(t, compressed, opts)
					t.Logf("Decompressed data 1: %x", decompressed)
					t.Logf("Compressed data 2: %x", compressed2)
					...
					decompressed2 := synchronousDecompressWriter(t, compressed2, opts)
					t.Logf("Decompressed data 2: %x", compressed2)

					if !bytes.Equal(decompressed, sample.decompressed) {
						t.Fatalf("decompressed != original data\n decompressed: %x\n original: %x", decompressed, sample.decompressed)
					}

					if !bytes.Equal(decompressed2, sample.decompressed) {
						t.Fatalf("decompressed2 != original data\n decompressed: %x\n original: %x", decompressed, sample.decompressed)
					}
				})
			})
		}
...
func TestCompressorWriterDecompressReader(t *testing.T) {
	samples := getDataSamples()
	optsList := getCombinations()
		...
		for _, opts := range optsList {
			...
			repeat(REPEAT_COUNT, func(index int) {
				...
				t.Run(name, func(t *testing.T) {
					...
					compressed, decompressed := asynchronousCompressAndDecompress(t, sample.decompressed, opts)
					t.Logf("Decompressed data 1: %x", decompressed)
					t.Logf("Compressed data 1: %x", compressed)

					compressed2 := synchronousCompressWriter(t, decompressed, opts)
					t.Logf("Compressed data 2: %x", compressed2)
...
func synchronousCompressReader(log testLogger, decompressed []byte, opts common.CompressOptions) []byte {
	...
	if err != nil {
		log.Fatalf("Error reading from compressor reader: %v", err)
	}
	return outCompressedData
}

// Test CompressorWriter synchronously when the full input is already available.
func synchronousCompressWriter(log testLogger, decompressed []byte, opts common.CompressOptions) []byte {
	var buf bytes.Buffer
...
func synchronousDecompressWriter(log testLogger, compressed []byte, opts common.CompressOptions) []byte {
	var buf bytes.Buffer
	decompressorWriter, err := zlib.NewDecompressWriter(&buf, decompressOptionsOfCompressOptions(opts))
	if err != nil {
...
func asynchronousDecompressAndCompress(log testLogger, compressed []byte, opts common.CompressOptions) (compressedResult []byte, decompressedResult []byte) {
	...
	decompressorWriter, err := zlib.NewDecompressWriter(multiWriter, decompressOptionsOfCompressOptions(opts))
	if err != nil {
		log.Fatalf("Error creating decompressor writer: %v", err)
	}
	...
	if compressReaderErr != nil && compressReaderErr != io.EOF {
		log.Fatalf("Error reading from compressor reader: %v", compressReaderErr)
	}
...
// by connecting them through a pipe.
// A side buffer is used to store the decompressed data.
func asynchronousCompressAndDecompress(log testLogger, decompressed []byte, opts common.CompressOptions) (compressedResult []byte, decompressedResult []byte) {
	...
	if err != nil {
		log.Fatalf("Error creating dcompressor writer: %v", err)
	}

	decompressorReader, err := zlib.NewDecompressReader(pipeReader, decompressOptionsOfCompressOptions(opts))
	if err != nil {
	...
	if decompressReaderErr != nil && decompressReaderErr != io.EOF {
		log.Fatalf("Error reading from decompressor reader: %v", decompressReaderErr)
	}

	return
}

func decompressOptionsOfCompressOptions(opts common.CompressOptions) common.DecompressOptions {
	return common.DefaultDecompressOptions().WithWindowBits(opts.WindowBits()).WithHeader(opts.Header())
}
